...
async function getAllUsers(req, res, next) {
    await database_1.default.user.findMany().then((val) => {
...
async function signUpUser(req, res, next) {
    ...
    await database_1.default.user.create({
        ...
        return (0, response_1.response)(res, 201, await Token.token_create(user.id), "token");
    }).catch((error) => {
...
async function verifyUser(req, res, next) {
    try {
        ...
        const userDatabase = await database_1.default.user.findUnique({
            where: { id }
        });
        if (!userDatabase) {
...
async function sendPasswordResetEmail(req, res, next) {
    ...
    user.sendPasswordReset().then(async () => {
        await database_1.default.user.update({ where: { email }, data: user.toJSON() });
